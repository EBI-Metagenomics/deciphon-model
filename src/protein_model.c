...
enum model_rc add_xnodes(struct protein_model *);
...
enum model_rc init_null_xtrans(struct imm_hmm *, struct protein_xnode_null *);
enum model_rc init_alt_xtrans(struct imm_hmm *, struct protein_xnode_alt *);
...
enum model_rc setup_entry_trans(struct protein_model *);
enum model_rc setup_exit_trans(struct protein_model *);
enum model_rc setup_transitions(struct protein_model *);

enum model_rc protein_model_add_node(struct protein_model *m,
                                     imm_float const lprobs[IMM_AMINO_SIZE],
                                     char consensus)
{
    if (!have_called_setup(m))
        return error(MODEL_EFAIL, "Must call protein_model_setup first.");

    if (m->alt.node_idx == m->core_size)
        return error(MODEL_EFAIL, "Reached limit of nodes.");
    ...
    struct protein_node *n = m->alt.nodes + m->alt.node_idx;

    setup_nuclt_dist(&n->match.nucltd, m->amino, m->code->nuclt, lodds);

    init_match(&n->M, m, &n->match.nucltd);
    if (imm_hmm_add_state(&m->alt.hmm, &n->M.super)) return MODEL_EFAIL;

    init_insert(&n->I, m);
    if (imm_hmm_add_state(&m->alt.hmm, &n->I.super)) return MODEL_EFAIL;

    init_delete(&n->D, m);
    if (imm_hmm_add_state(&m->alt.hmm, &n->D.super)) return MODEL_EFAIL;
...
enum model_rc protein_model_add_trans(struct protein_model *m,
                                      struct protein_trans trans)
{
    ...
    if (m->alt.trans_idx == m->core_size + 1)
        return error(MODEL_EFAIL, "Reached limit of transitions.");
...
void protein_model_init(struct protein_model *m, struct imm_amino const *amino,
...
enum model_rc protein_model_setup(struct protein_model *m, unsigned core_size)
{
    ...
    if (!ptr && n > 0) return error(MODEL_ENOMEM, "failed to alloc nodes");
    m->alt.nodes = ptr;

    if (m->cfg.entry_dist == ENTRY_DIST_OCCUPANCY)
    {
        ptr = realloc(m->alt.locc, n * sizeof(*m->alt.locc));
        if (!ptr && n > 0) return error(MODEL_ENOMEM, "failed to alloc locc");
        m->alt.locc = ptr;
    }
    m->alt.trans_idx = 0;
    ptr = realloc(m->alt.trans, (n + 1) * sizeof(*m->alt.trans));
    if (!ptr) return error(MODEL_ENOMEM, "failed to alloc trans");
    m->alt.trans = ptr;
...
enum model_rc add_xnodes(struct protein_model *m)
{
    struct protein_xnode *n = &m->xnode;

    if (imm_hmm_add_state(&m->null.hmm, &n->null.R.super)) return MODEL_EFAIL;
    if (imm_hmm_set_start(&m->null.hmm, &n->null.R.super, LOG1))
        return MODEL_EFAIL;

    if (imm_hmm_add_state(&m->alt.hmm, &n->alt.S.super)) return MODEL_EFAIL;
    if (imm_hmm_add_state(&m->alt.hmm, &n->alt.N.super)) return MODEL_EFAIL;
    if (imm_hmm_add_state(&m->alt.hmm, &n->alt.B.super)) return MODEL_EFAIL;
    if (imm_hmm_add_state(&m->alt.hmm, &n->alt.E.super)) return MODEL_EFAIL;
    if (imm_hmm_add_state(&m->alt.hmm, &n->alt.J.super)) return MODEL_EFAIL;
    if (imm_hmm_add_state(&m->alt.hmm, &n->alt.C.super)) return MODEL_EFAIL;
    if (imm_hmm_add_state(&m->alt.hmm, &n->alt.T.super)) return MODEL_EFAIL;
...
{
    struct protein_trans *trans = m->alt.trans;
...
enum model_rc init_null_xtrans(struct imm_hmm *hmm,
...
enum model_rc init_alt_xtrans(struct imm_hmm *hmm, struct protein_xnode_alt *n)
{
...
enum model_rc setup_entry_trans(struct protein_model *m)
{
...
enum model_rc setup_exit_trans(struct protein_model *m)
{
...
enum model_rc setup_transitions(struct protein_model *m)
{
    ...
    if (imm_hmm_set_trans(h, B, M1, trans[0].MM)) return MODEL_EFAIL;

    for (unsigned i = 0; i + 1 < m->core_size; ++i)
    {
        struct protein_node *pr = m->alt.nodes + i;
        struct protein_node *nx = m->alt.nodes + i + 1;
        unsigned j = i + 1;
    ...
    struct imm_state *E = &m->xnode.alt.E.super;
    if (imm_hmm_set_trans(h, Mm, E, trans[n].MM)) return MODEL_EFAIL;

    if (setup_entry_trans(m)) return MODEL_EFAIL;
    if (setup_exit_trans(m)) return MODEL_EFAIL;
    if (init_null_xtrans(&m->null.hmm, &m->xnode.null)) return MODEL_EFAIL;
