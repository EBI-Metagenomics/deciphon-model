#include "deciphon_model/standard_profile.h"
...
{
    struct standard_profile *p = (struct standard_profile *)prof;
    if (imm_dp_unpack(&p->dp.null, file)) return MODEL_EFAIL;
    if (imm_dp_unpack(&p->dp.alt, file)) return MODEL_EFAIL;
    return MODEL_OK;
}
                                     ...
                                     struct lip_file *file)
{
    ...
    if (imm_dp_pack(&prof->dp.null, file)) return MODEL_EFAIL;

    if (!lip_write_cstr(file, "alt")) return eio("write alt state key");
    if (imm_dp_pack(&prof->dp.alt, file)) return MODEL_EFAIL;
